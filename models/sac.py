...
from utils.utils_sac import soft_update, hard_update
from models.model_sac import GaussianPolicy, QNetwork, DeterministicPolicy
...
class SAC(object):
    def __init__(self, num_inputs, action_space, args_dict):
        ...
        if self.policy_type == "Gaussian":
            ...
            if self.automatic_entropy_tuning is True:
                self.target_entropy = -0.5*torch.prod(torch.Tensor(action_space.shape).to(self.device)).item()
    ...
    def update_parameters(self, memory, batch_size, updates, discr=None):
        ...
        if discr:
            reward_batch = discr.predict_reward(
                state_batch, action_batch, None, None, update_rms=False, use_disc_as_adv=False)
        with torch.no_grad():
